...
public class CobroServices(Contexto contexto)
{
	...
	{
		await AfectarCobro(cobros.CobrosDetalles.ToArray());
	...
	{
		var cobro =  _contexto.Cobros.Find(id);
		
		await AfectarCobro(cobro.CobrosDetalles.ToArray(), false);

		_contexto.CobrosDetalles.RemoveRange(cobro.CobrosDetalles);
	...
	private async Task AfectarCobro(CobrosDetalles[] detalles, bool resta = true)
	{

		foreach (var item in detalles)
		{
			var prestamo = await _contexto.Prestamos.SingleAsync(p => p.PrestamoId == item.PrestamoId);
			if (resta)
			{
